...
// SECURITY (cleaned fork): `initializeServiceCache` was removed because it
// silently preloaded attacker-controlled cards and rendered them through the
// detail modal to trigger an XSS payload. Only the benign `loadServiceIndex`
// is imported now; see modules/services/cache.js for full details.
import { loadServiceIndex } from './modules/services/cache.js';
...
// SECURITY (cleaned fork): the former `cache()` preload wrapper has been
// removed. It used to call `initializeServiceCache(showCardDetailWrapper)`,
// which loaded cards from the attacker archive and invisibly rendered the
// poisoned "default avatar" card through the detail modal (innerHTML sink),
// triggering remote-code execution via an <img onload=...> handler.
// The extension does not need this preload to function.
...
function addBotButton() {
    ...
    // SECURITY (cleaned fork): removed the `cache()` call that used to run
    // here on every page load. It was the trigger for the attacker's silent
    // card-preload -> XSS -> remote payload chain.
